...
/*\ 管理模块，处理上下文，相当于lua中的虚拟机 \*/
struct skynet_context {
	void * instance;				/*\ 由指定module的create函数，创建的数据实例指针，同一类服务可能有多个实例，每个服务都有自己的数据 \*/
	struct skynet_module * mod;		/*\ 引用服务module的指针，用来调用create,init,release,signal函数 \*/
	void * cb_ud;					/*\ 调用回调函数时候，回传给callback的userdata，一般是instance指针 \*/
	skynet_cb cb;					/*\ 消息处理的回调函数，一般在skynet_module 中的init函数中指定 \*/
	struct message_queue *queue;	/*\ 服务专属的次级消息队列指针 \*/
	FILE * logfile;					/*\ 日志服务器的文件句柄 \*/
	uint64_t cpu_cost;	// in microsec
	uint64_t cpu_start;	// in microsec
	char result[32];				/*\ 操作skynet_context的返回值 \*/
	uint32_t handle;				/*\ 标识唯一skynet_context服务器id \*/
	int session_id;					/*\ 发送消息时候带着，用来判断是否是对应的处理后数据，返回数据的时候根据session_id去匹配是哪个消息 \*/
	int ref;						/*\ 引用计数变量，为0的时候可以释放内存 \*/
