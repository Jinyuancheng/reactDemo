...
bool generate_image_thumbnail(const char* inputFile, const char* outputFile)
{
	...
	int tb_w = 540, tb_h = 960;
	unsigned char * img_buf = do_Stretch_Linear(tb_h, tb_w, 24, buff, w, h);
	free(buff);

	//将缩放后的像素数组保存到jpeg文件
	bool bRetWrite = write_JPEG_file(outputFile, img_buf, 70, tb_w, tb_h);
	free(img_buf);
...
{
	const char* inFile = "/usr/local/jpeg/01.jpg";
	const char* outFile = "/usr/local/jpeg/01_copy.jpg";
	if (generate_image_thumbnail(inFile, outFile))
	{
		printf("generate_image_thumbnail success\n");
	}
